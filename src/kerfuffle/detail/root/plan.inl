...
Status::Status(const Result result_, const bool outerTransition_)
...
ConstPlanT<TArgs>::Iterator::operator bool() const {
  KERFUFFLE_ASSERT(_curr < ConstPlanT::TASK_CAPACITY ||
                   _curr == INVALID_LONG_INDEX);
...
template <typename TArgs>
void ConstPlanT<TArgs>::Iterator::operator++() {
  _curr = _next;
  _next = next();
}
...
template <typename TArgs>
LongIndex ConstPlanT<TArgs>::Iterator::next() const {
  if (_curr < ConstPlanT::TASK_CAPACITY) {
    const TaskLink& task = _plan._planData.taskLinks[_curr];

    return task.next;
  } else {
    KERFUFFLE_ASSERT(_curr == INVALID_LONG_INDEX);
...
template <typename TArgs>
ConstPlanT<TArgs>::ConstPlanT(const PlanData& planData, const RegionID regionId)

    : _planData{planData}, _bounds{planData.tasksBounds[regionId]} {}
...
ConstPlanT<TArgs>::operator bool() const {
  if (_bounds.first < TASK_CAPACITY) {
    KERFUFFLE_ASSERT(_bounds.last < TASK_CAPACITY);
    return true;
  } else {
    KERFUFFLE_ASSERT(_bounds.last == INVALID_LONG_INDEX);
    return false;
  }
}
...
PlanT<TArgs>::Iterator::operator bool() const {
  KERFUFFLE_ASSERT(_curr < PlanT::TASK_CAPACITY || _curr == INVALID_LONG_INDEX);
...
template <typename TArgs>
void PlanT<TArgs>::Iterator::operator++() {
  _curr = _next;
  _next = next();
}
...
template <typename TArgs>
void PlanT<TArgs>::Iterator::remove() {
  _plan.remove(_curr);
}
...
template <typename TArgs>
LongIndex PlanT<TArgs>::Iterator::next() const {
...
template <typename TArgs>
PlanT<TArgs>::PlanT(PlanData& planData, const RegionID regionId)
...
template <typename TArgs>
bool PlanT<TArgs>::append(const TransitionType transitionType,
                          const StateID        origin,
                          const StateID        destination) {
  _planData.planExists.set(_regionId);

  const TaskIndex index =
      _planData.taskLinks.emplace(transitionType, origin, destination);
  if (index == TaskLinks::INVALID)
    return false;
  else {
    if (_bounds.first < TaskLinks::CAPACITY) {
      KERFUFFLE_ASSERT(_bounds.last < TaskLinks::CAPACITY);
      ...
      KERFUFFLE_ASSERT(_bounds.first == INVALID_LONG_INDEX &&
...
template <typename TArgs>
void PlanT<TArgs>::clear() {
  if (_bounds.first < TaskLinks::CAPACITY) {
    KERFUFFLE_ASSERT(_bounds.last < TaskLinks::CAPACITY);

    for (LongIndex index = _bounds.first; index != INVALID_LONG_INDEX;) {
      KERFUFFLE_ASSERT(index < TaskLinks::CAPACITY);

      const auto& task = _planData.taskLinks[index];
      KERFUFFLE_ASSERT(index == _bounds.first
...
template <typename TArgs>
void PlanT<TArgs>::remove(const LongIndex task) {
  KERFUFFLE_ASSERT(_planData.planExists.get(_regionId) &&
                   _bounds.first < TaskLinks::CAPACITY &&
                   _bounds.last < TaskLinks::CAPACITY);

  KERFUFFLE_ASSERT(task < TaskLinks::CAPACITY);
  ...
  if (curr.prev < TaskLinks::CAPACITY) {
    ...
    KERFUFFLE_ASSERT(_bounds.first == task);
  ...
  if (curr.next < TaskLinks::CAPACITY) {
    ...
    KERFUFFLE_ASSERT(_bounds.last == task);
