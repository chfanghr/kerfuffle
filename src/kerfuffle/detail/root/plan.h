...
inline Status combine(const Status lhs, const Status rhs) {
...
class ConstPlanT {
 ...
 public:
  ...
  struct Iterator {
    KERFUFFLE_INLINE Iterator(const ConstPlanT& plan);

    KERFUFFLE_INLINE explicit operator bool() const;

    KERFUFFLE_INLINE void operator++();

    KERFUFFLE_INLINE const TaskLink& operator*() const {
      return _plan._planData.taskLinks[_curr];
    }
    KERFUFFLE_INLINE const TaskLink* operator->() const {
      return &_plan._planData.taskLinks[_curr];
    }

    KERFUFFLE_INLINE LongIndex next() const;
 ...
 private:
  KERFUFFLE_INLINE ConstPlanT(const PlanData& planData,
 ...
 public:
  ...
  KERFUFFLE_INLINE Iterator first() { return Iterator{*this}; }
...
class PlanT {
 ...
 public:
  ...
  struct Iterator {
    KERFUFFLE_INLINE Iterator(PlanT& plan);

    KERFUFFLE_INLINE explicit operator bool() const;

    KERFUFFLE_INLINE void operator++();

    KERFUFFLE_INLINE TaskLink& operator*() {
      return _plan._planData.taskLinks[_curr];
    }
    KERFUFFLE_INLINE const TaskLink& operator*() const {
      return _plan._planData.taskLinks[_curr];
 ...
 private:
  KERFUFFLE_INLINE PlanT(PlanData& planData, const RegionID regionId);
 ...
 public:
  ...
  ///		(if transitioning into a region, acts depending on the region
  ///type)
  ...
  KERFUFFLE_INLINE bool change(const StateID origin,
                               const StateID destination) {
    return append(TransitionType::CHANGE, origin, destination);
  }
  ...
  /// @return Seccess if FSM total number of tasks is below task capacity
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  KERFUFFLE_INLINE bool restart(const StateID origin,
                                const StateID destination) {
    return append(TransitionType::RESTART, origin, destination);
  }
  ...
  /// @return Seccess if FSM total number of tasks is below task capacity
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  KERFUFFLE_INLINE bool resume(const StateID origin,
                               const StateID destination) {
    return append(TransitionType::RESUME, origin, destination);
  }
...
#  ifdef KERFUFFLE_ENABLE_UTILITY_THEORY
  ...
  KERFUFFLE_INLINE bool utilize(const StateID origin,
                                const StateID destination) {
    return append(TransitionType::UTILIZE, origin, destination);
  }
  ...
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  /// @see KERFUFFLE_ENABLE_UTILITY_THEORY
  KERFUFFLE_INLINE bool randomize(const StateID origin,
                                  const StateID destination) {
    return append(TransitionType::RANDOMIZE, origin, destination);
  }
...
#  endif
  ...
  /// @return Seccess if FSM total number of tasks is below task capacity
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  KERFUFFLE_INLINE bool schedule(const StateID origin,
                                 const StateID destination) {
    return append(TransitionType::SCHEDULE, origin, destination);
  }
  ...
  KERFUFFLE_INLINE bool change(const StateID destination) {
  ...
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  template <typename TOrigin>
  KERFUFFLE_INLINE bool restart(const StateID destination) {
  ...
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  template <typename TOrigin>
  KERFUFFLE_INLINE bool resume(const StateID destination) {
...
#  ifdef KERFUFFLE_ENABLE_UTILITY_THEORY
  ...
  /// @see KERFUFFLE_ENABLE_UTILITY_THEORY
  template <typename TOrigin>
  KERFUFFLE_INLINE bool utilize(const StateID destination) {
  ...
  /// @see KERFUFFLE_ENABLE_UTILITY_THEORY
  template <typename TOrigin>
  KERFUFFLE_INLINE bool randomize(const StateID destination) {
...
#  endif
  ...
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  template <typename TOrigin>
  KERFUFFLE_INLINE bool schedule(const StateID destination) {
...
#  ifdef KERFUFFLE_ENABLE_UTILITY_THEORY
  ...
  /// @see KERFUFFLE_ENABLE_UTILITY_THEORY
  template <typename TOrigin, typename TDestination>
  KERFUFFLE_INLINE bool randomize() {
...
#  endif
  ...
  /// @note use 'Config::TaskCapacityN<>' to increase task capacity if necessary
  template <typename TOrigin, typename TDestination>
  KERFUFFLE_INLINE bool schedule() {
