...
struct alignas(2 * sizeof(ShortIndex)) Parent {
  KERFUFFLE_INLINE Parent() = default;

  KERFUFFLE_INLINE Parent(const ForkID forkId_) : forkId{forkId_} {}

  KERFUFFLE_INLINE Parent(const ForkID forkId_, const ShortIndex prong_)
      : forkId{forkId_}, prong{prong_} {}

  KERFUFFLE_INLINE explicit operator bool() const {
    return forkId != INVALID_FORK_ID && prong != INVALID_SHORT_INDEX;
  }
...
struct Request {
  ...
  KERFUFFLE_INLINE Request() = default;

  KERFUFFLE_INLINE Request(const Type type_, const StateID stateId_)
      : type{type_}, stateId{stateId_} {
    KERFUFFLE_ASSERT(type_ < Type::COUNT);
  }
...
template <typename,
          ...
          LongIndex,
          LongIndex,
          LongIndex KERFUFFLE_IF_PLANS(, LongIndex)>
struct ArgsT;
...
struct RegistryT<ArgsT<TContext,
                       ...
                       NOrthoCount,
                       NOrthoUnits,
                       NSerialBits KERFUFFLE_IF_PLANS(, NTaskCapacity)>> {
  ...
  KERFUFFLE_INLINE const Parent& forkParent(const ForkID forkId) const;

  KERFUFFLE_INLINE OrthoBits requestedOrthoFork(const ForkID forkId);
...
template <typename TRegistry>
void backup(const TRegistry& registry, BackUpT<TRegistry>& copy) {
...
template <typename TRegistry>
void restore(TRegistry& registry, const BackUpT<TRegistry>& copy) {
