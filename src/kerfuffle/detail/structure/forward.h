...
struct CSI_<TInitial, TRemaining...> {
  ...
  static constexpr LongIndex REVERSE_DEPTH =
      Max<Initial::REVERSE_DEPTH, Remaining::REVERSE_DEPTH>::VALUE;
  static constexpr ShortIndex COMPO_REGIONS =
      Initial::COMPO_REGIONS + Remaining::COMPO_REGIONS;
  static constexpr LongIndex COMPO_PRONGS =
      Initial::COMPO_PRONGS + Remaining::COMPO_PRONGS;
  static constexpr ShortIndex ORTHO_REGIONS =
      Initial::ORTHO_REGIONS + Remaining::ORTHO_REGIONS;
  static constexpr ShortIndex ORTHO_UNITS =
      Initial::ORTHO_UNITS + Remaining::ORTHO_UNITS;

  static constexpr LongIndex ACTIVE_BITS =
      Max<Initial::ACTIVE_BITS, Remaining::ACTIVE_BITS>::VALUE;
  static constexpr LongIndex RESUMABLE_BITS =
...
struct OI_ final {
  ...
  static constexpr ShortIndex ORTHO_UNITS =
      SubStates::ORTHO_UNITS + (WIDTH + 7) / 8;
...
template <typename TContext,
          ...
          LongIndex NSerialBits KERFUFFLE_IF_PLANS(, LongIndex NTaskCapacity)>
struct ArgsT final {
  ...
  KERFUFFLE_IF_STRUCTURE_REPORT(
      using StructureStateInfos = Array<StructureStateInfo, STATE_COUNT>);
};
...
struct RF_ final {
...
#ifdef KERFUFFLE_ENABLE_PLANS
  static constexpr LongIndex TASK_CAPACITY =
      TConfig::TASK_CAPACITY != INVALID_LONG_INDEX ? TConfig::TASK_CAPACITY
                                                   : Apex::COMPO_PRONGS * 2;
#endif
  ...
  using Args =
      ArgsT<Context,
            ...
            ORTHO_REGIONS,
            ORTHO_UNITS,
            ACTIVE_BITS + RESUMABLE_BITS KERFUFFLE_IF_PLANS(, TASK_CAPACITY)>;
