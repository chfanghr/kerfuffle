...
template <typename TI, ShortIndex NC>
void BitArray<TI, NC>::Bits::clear() {
  const Index count = (_width + 7) / (sizeof(Unit) * 8);

  for (Index i = 0; i < count; ++i) _storage[i] = Unit{0};
}
...
bool BitArray<TI, NC>::Bits::get() const {
  constexpr Index INDEX = NIndex;
  KERFUFFLE_ASSERT(INDEX < _width);
...
template <typename TI, ShortIndex NC>
template <ShortIndex NIndex>
void BitArray<TI, NC>::Bits::reset() {
  constexpr Index INDEX = NIndex;
  KERFUFFLE_ASSERT(INDEX < _width);
...
template <typename TI, ShortIndex NC>
bool BitArray<TI, NC>::Bits::get(const Index index) const {
  KERFUFFLE_ASSERT(index < _width);
...
template <typename TI, ShortIndex NC>
void BitArray<TI, NC>::Bits::set(const Index index) {
  KERFUFFLE_ASSERT(index < _width);
...
template <typename TI, ShortIndex NC>
void BitArray<TI, NC>::Bits::reset(const Index index) {
  KERFUFFLE_ASSERT(index < _width);
...
template <typename TI, ShortIndex NC>
template <ShortIndex NIndex>
bool BitArray<TI, NC>::ConstBits::get() const {
  constexpr Index INDEX = NIndex;
...
template <typename TI, ShortIndex NC>
bool BitArray<TI, NC>::ConstBits::get(const Index index) const {
  KERFUFFLE_ASSERT(index < _width);
...
template <typename TI, ShortIndex NC>
void BitArray<TI, NC>::clear() {
  for (Unit& unit : _storage) unit = Unit{0};
}
...
template <typename TI, ShortIndex NC>
bool BitArray<TI, NC>::get(const Index index) const {
  KERFUFFLE_ASSERT(index < CAPACITY);
...
template <typename TI, ShortIndex NC>
void BitArray<TI, NC>::set(const Index index) {
  KERFUFFLE_ASSERT(index < CAPACITY);
...
typename BitArray<TI, NC>::Bits BitArray<TI, NC>::bits() {
...
template <typename TI, ShortIndex NC>
template <ShortIndex NUnit, ShortIndex NWidth>
typename BitArray<TI, NC>::ConstBits BitArray<TI, NC>::bits() const {
  constexpr ShortIndex UNIT  = NUnit;
  constexpr ShortIndex WIDTH = NWidth;
  static_assert(UNIT + (WIDTH + 7) / (sizeof(Unit) * 8) <= CAPACITY, "");
...
template <typename TI, ShortIndex NC>
typename BitArray<TI, NC>::Bits BitArray<TI, NC>::bits(const Units& units) {
  KERFUFFLE_ASSERT(units.unit + (units.width + 7) / (sizeof(Unit) * 8) <=
                   CAPACITY);
...
template <typename TI, ShortIndex NC>
typename BitArray<TI, NC>::ConstBits BitArray<TI, NC>::bits(
