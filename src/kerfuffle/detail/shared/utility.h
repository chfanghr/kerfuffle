...
template <typename T>
KERFUFFLE_INLINE void fill(T& a, const char value) {
...
constexpr unsigned count(const T (&)[NCount]) {
  return NCount;
}
...
struct UnsignedCapacityT {
  ...
  using Type = typename std::conditional<
      ...
      typename std::conditional<
          CAPACITY <= UINT16_MAX,
          uint16_t,
          typename std::conditional<CAPACITY <= UINT32_MAX,
                                    uint32_t,
...
struct UnsignedBitWidthT {
  ...
  using Type = typename std::conditional<
      ...
      typename std::conditional<
          BIT_WIDTH <= 16,
          uint16_t,
          typename std::conditional<BIT_WIDTH <= 32, uint32_t, uint64_t>::
              type>::type>::type;
...
constexpr LongIndex roundUp(const LongIndex x, const LongIndex to) {
  return (x + (to - 1)) / to;
}

//------------------------------------------------------------------------------

constexpr ShortIndex bitWidth(const ShortIndex x) {
  return x <= 2     ? 1
         : x <= 4   ? 2
         : x <= 8   ? 3
         : x <= 16  ? 4
...
void overwrite(TTo& to, const TFrom& from) {
