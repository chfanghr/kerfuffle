...
template <LongIndex NBC>
BitWriteStream<NBC>::BitWriteStream(Buffer& buffer) : _buffer{buffer} {
  _buffer.clear();
}
...
void BitWriteStream<NBC>::write(const UnsignedBitWidth<NBitWidth> item) {
  constexpr ShortIndex BIT_WIDTH = NBitWidth;
  static_assert(BIT_WIDTH > 0, "STATIC ASSERT");

  KERFUFFLE_ASSERT(_buffer.bitSize + BIT_WIDTH < BIT_CAPACITY);
...
template <LongIndex NBC>
template <ShortIndex NBitWidth>
UnsignedBitWidth<NBitWidth> BitReadStream<NBC>::read() {
  constexpr ShortIndex BIT_WIDTH = NBitWidth;
  static_assert(BIT_WIDTH > 0, "STATIC ASSERT");
  ...
  for (ShortIndex itemWidth = BIT_WIDTH; itemWidth;)
    if (KERFUFFLE_CHECKED(_cursor + itemWidth <= _buffer.bitSize)) {
