...
struct MakeIndexSequenceT<IndexConstant<N>, IndexSequence<Ns...>>
    : MakeIndexSequenceT<IndexConstant<N - 1>, IndexSequence<N - 1, Ns...>> {};

template <LongIndex... Ns>
struct MakeIndexSequenceT<IndexConstant<0>, IndexSequence<Ns...>>
    : IndexSequence<Ns...> {};
...
using MakeIndexSequence =
    typename MakeIndexSequenceT<IndexConstant<N>, IndexSequence<>>::Type;
...
struct ITL_EntryN : ITL_EntryT<T> {};
...
struct ITL_Impl<IndexSequence<Ns...>, Ts...> : ITL_EntryN<Ts, Ns>... {
  template <typename T, LongIndex N>
  static constexpr LongIndex select(ITL_EntryN<T, N>) {
    return (LongIndex)N;
  }
};
...
struct ITL_ : private ITL_Impl<IndexSequenceFor<Ts...>, Ts...> {
  ...
  static constexpr bool contains() {
    return std::is_base_of<ITL_EntryT<T>, ITL_>::value;
  }

  template <typename T>
  static constexpr
      typename std::enable_if<std::is_base_of<ITL_EntryT<T>, ITL_>::value,
                              LongIndex>::type
      index() {
